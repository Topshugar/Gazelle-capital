...
<script src="https://quge5.com/88/tag.min.js" data-zone="282065" async data-cfasync="false"></script>
<style>
...
def home():
    ...
    <div class='ticker'><div class='ticker-content'>
        ...
        <span>📈 GOLD +1.24% TODAY</span>
        <span>💰 BTC $68,420</span>
        <span>📊 NASDAQ +0.82%</span>
        <span>⚡ GAZELLE AI: 84% WIN RATE THIS WEEK</span>
        <span>🔥 1,247 TRADERS ONLINE</span>
        <span>🤖 ALL BROKERS SUPPORTED - XM HFM EXNESS DERIV</span>
    </div></div>
    <div class='hero'>
        <h1 style='font-size:36px;margin:10px 0'>GAZELLE CAPITAL</h1>
        <p style='color:#00ff88;font-size:13px;font-weight:bold'>CONNECT ANY BROKER • AUTO TRADING • 84% WIN RATE • PROP FIRM APPROVED</p>
        <div class='stats'>
            <div class='stat'><h2>${gold}</h2><p>XAUUSD LIVE</p></div>
            <div class='stat'><h2>84%</h2><p>WIN RATE</p></div>
            <div class='stat'><h2>$2.4M+</h2><p>PAYOUTS</p></div>
        </div>
        ...
        <div class='box' style='border:2px solid gold;background:linear-gradient(135deg,#1a1a00,#000)'>
            <h2 style='margin:0;font-size:22px'>Auto Trade Your Broker</h2>
            <p style='color:#888;font-size:12px'>Demo $100 free • Connect XM/HFM/Exness/Deriv any MT5 • Bot trades for you</p>
            <a href='/register'><button>Connect Broker Free Demo</button></a>
            <p style='font-size:10px;color:#666;margin-top:10px'>Have account? <a href='/login'>Login</a></p>
        </div>
    </div>
    <div class='news-section'>
        <h3 style='color:gold;font-size:14px'>🔴 LIVE MARKET NEWS</h3>
        <div class='news-card'><span style='background:#ff0000;color:#fff;padding:3px 6px;border-radius:4px;font-size:9px'>BREAKING • {now}</span><h4 style='margin:5px 0;font-size:13px'>Gold Hits ${gold} - Fed Rate Decision Looms</h4><p style='color:#888;font-size:11px'>XAUUSD volatility expected. Gazelle AI positioned for breakout - auto trading active on all brokers.</p></div>
        <div class='news-card'><span style='background:#00ff88;color:#000;padding:3px 6px;border-radius:4px;font-size:9px'>PROFIT • 2m ago</span><h4 style='margin:5px 0;font-size:13px'>VIP Member Cashout $4,820 Today - Exness MT5</h4><p style='color:#888;font-size:11px'>Private bot triggered 3 TP hits in London session. Auto-copied to his broker.</p></div>
        <div class='news-card'><span style='background:gold;color:#000;padding:3px 6px;border-radius:4px;font-size:9px'>AI SIGNAL • 5m ago</span><h4 style='margin:5px 0;font-size:13px'>Gazelle AI Detected High Probability Setup - XAUUSD</h4><p style='color:#888;font-size:11px'>Login to connect your broker and copy trades automatically. No setups shown - profit only.</p></div>
    </div>
    </body></html>
    """
    return html
...
def dashboard():
    ...
    if is_vip:
        if mt5_connected:
            xm_box = f"<div class='box' style='border:2px solid #00ff88'><h3 style='color:#00ff88'>✅ {broker} CONNECTED</h3><p style='font-size:11px'>Broker: {broker}<br>Login: {mt5_login}<br>Server: {mt5_server}<br>Risk: {risk} lot<br><br>Bot trading your account automatically (simulated now, real VPS later).</p><form method='post' action='/update-risk'><select name='risk'><option value='0.01'>0.01 Low Risk</option><option value='0.05' {'selected' if risk=='0.05' else ''}>0.05 Medium</option><option value='0.10'>0.10 High</option><option value='0.20'>0.20 Aggressive</option></select><button>Update Risk</button></form><br><a href='/broker-disconnect'><button style='background:#ff4444'>Disconnect Broker</button></a></div>"
        else:
            xm_box = f"""
            <div class='box' style='border:2px solid gold'>
                <h3 style='color:gold'>🔌 Connect Any MT5 Broker</h3>
                <p style='font-size:10px;color:#888'>XM, HFM, Exness, Deriv, FBS, OctaFX, any MT5</p>
                <form method='post' action='/broker-connect'>
                    ...
                    <button>Connect & Start Auto Trading</button>
                </form>
            </div>
            """
    else:
        xm_box = f"<div class='box' style='border:2px solid #ffaa00;background:#1a1500'><h3 style='color:#ffaa00'>🔒 DEMO MODE</h3><p style='font-size:11px'>Demo $100 simulated - can't withdraw. Subscribe to connect your real broker.</p><a href='/subscribe'><button style='background:#ffaa00'>Become VIP - Connect Broker</button></a></div>"

    return f"<html><head>{CSS}</head><body style='display:block'><div style='max-width:500px;margin:15px auto'><h1 style='text-align:center'>{'LIVE BOT' if is_vip and mt5_connected else 'DEMO BOT'}</h1><p style='text-align:center;color:#888'>${gold} | {now}</p>{xm_box}<div class='box'><h2 style='color:#00ff88;font-size:32px;margin:0'>${demo_bal}</h2><p>{trend} | {direction_txt} | {status}</p><p style='font-size:9px;color:#555'>Secret AI - No setups shown - Profit Only</p></div><div class='box'><h3 style='color:gold'>Trade Log (Profit Only)</h3>{trades_html}</div><div class='box'><a href='/logout'>Logout</a> | <a href='/'>Home</a></div></div></body></html>"
